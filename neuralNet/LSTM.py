...
def train_loop(model: LSTMclassifier, dataloader, loss_fun, optimizer, epochs):
    model.train()
    model.to(device)
    lossList = []
    accuracyList = []
...
if __name__ == "__main__":
    ...
    if test_accuracy > 0.65:
        torch.save(net, "./model/转子模型/LSTM_bear.pt")
    ...
    axe[0].set_title("LSTM模型训练集损失", font={"family": "SimHei", "size": 16})
    ...
    axe[1].set_title("LSTM模型训练集精确度", font={"family": "SimHei", "size": 16})
    plt.tight_layout()
    plt.show()
